...
it('paginates motherboard', async () => {
  for (let i = 0; i < 6; i++) {
    await createMotherboard()
  }

  const firstPage = await request(app)
    .get(`/api/parts/motherboard/`)
  ...
  const secondPage = await request(app)
    .get(`/api/parts/motherboard/?page=2`)
    .set('Cookie', global.signin())
    .send()
    .expect(200)
  ...
  const thirdPage = await request(app)
    .get(`/api/parts/motherboard/?page=3`)
    .set('Cookie', global.signin())
    .send()
    .expect(200)
...
it('paginate to page 1 if provided invalid query', async () => {
  ...
  const firstPage = await request(app)
    .get(`/api/parts/motherboard/?page=1`)
    .set('Cookie', global.signin())
    .send()
    .expect(200)
