...
it('paginates cpu cooler', async () => {
  for (let i = 0; i < 6; i++) {
    await createCpuCooler()
  }

  const firstPage = await request(app)
    .get(`/api/parts/cpucooler/`)
  ...
  const secondPage = await request(app)
    .get(`/api/parts/cpucooler/?page=2`)
    .set('Cookie', global.signin())
    .send()
    .expect(200)
  ...
  const thirdPage = await request(app)
    .get(`/api/parts/cpucooler/?page=3`)
    .set('Cookie', global.signin())
    .send()
    .expect(200)
...
it('paginate to page 1 if provided invalid query', async () => {
  ...
  const firstPage = await request(app)
    .get(`/api/parts/cpucooler/?page=qweqweqwe`)
    .set('Cookie', global.signin())
    .send()
    .expect(200)
